...
8. 이제 pr과 pl을 비교한다.
pr이 더 작아졌다는 의미는 정렬된 원소 중 삽입해야 할 원소보다 더 큰 값 혹은 작은 값이 반복적으로 존재한다는 의미이며, pl이 pr보다 작거나 같다는 것은 정렬된 원소 중 동일한 원소가 존재한다는 의미이다.
...
9. 다음으로 원소 i부터 pd-1 까지 반복문을 돌리며, 삽입해야할 원소 뒤 원소들은 밀어주고 그 자리에 본인을 삽입한다.
